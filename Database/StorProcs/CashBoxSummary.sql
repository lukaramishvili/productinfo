...
-- =============================================
CREATE PROCEDURE [dbo].[CashBoxSummary] 
...
BEGIN
	...
	SET @total_taken = (SELECT SUM(mt_taken.taken_amt)
	FROM
	 (	SELECT SUM(money_transfer.amount) AS taken_amt,money_transfer.client_type
				...
				WHERE	money_transfer.type='Take' 
						...
						AND money_transfer.cashbox_id = @CashBoxID
				GROUP by money_transfer.client_type
	) mt_taken)
	
	SET @total_given = (SELECT SUM(mt_given.given_amt)
	FROM 
